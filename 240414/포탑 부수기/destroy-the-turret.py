...
def find_fighter():
    ...
    min_power_list = sorted(
        min_power_list,
        key=lambda x: (x[2], -x[3], -(x[0] + x[1]), -x[1]),  # 정렬 조건 오류
    )
...
def raser(fighter, target):
    ...
    while x != fighter.x or y != fighter.y:
        ...
        tar.power = max(0, tar.power - minus_power)
        isEffect[x][y] = True
...
def bomb(fighter, target):
    target.power = max(0, target.power - fighter.power)
    ##### 포탄 공격 대상 자체에게는 영향을 주지 않았음!!!1
    ...
    for d in range(8):
        ...
        if (nx, ny) == (fighter.x, fighter.y):
            continue

        if (nx, ny) == (target.x, target.y):
            minus_power = fighter.power

        graph[nx][ny].power = max(0, graph[nx][ny].power - minus_power)
        isEffect[nx][ny] = True
...
for turn in range(1, k + 1):
    ...
    for i in range(1, n + 1):
        for j in range(1, m + 1):
            if graph[i][j].power > 0 and not isEffect[i][j]:
                graph[i][j].power += 1
