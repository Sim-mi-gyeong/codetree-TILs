...
def interaction(src_santa_num, target_santa_num, next_x, next_y, dir):

    new_next_x = next_x + dx[dir]
    new_next_y = next_y + dy[dir]
...
# 산타에 의해 밀려난건지 -> type : 1
# 에 따라 dx 가 달라짐
def conflict(score, num, x, y, dir):
    ...
    next_x = santas[num].x + dx[dir] * score
    next_y = santas[num].y + dy[dir] * score
    ...
    if graph[next_x][next_y] != 0:
        interaction(num, graph[next_x][next_y], next_x, next_y, dir)
...
def move_rudolf(turn):
    ...
    for d in range(8):
        next_x, next_y = rudolf.x + dx[d], rudolf.y + dy[d]
        if next_x < 1 or next_x > n or next_y < 1 or next_y > n:
            continue
    ...
    if graph[rudolf.x][rudolf.y] != 0:
        ...
        conflict(c, santa_num, santas[santa_num].x, santas[santa_num].y, min_dir)
...
def move_santa(turn, idx):
    ...
    min_dir_list = []
    for i in range(4):
        santa_next_x = santas[idx].x + dx[i]
        santa_next_y = santas[idx].y + dy[i]

        # print(idx, " 번 산타 이동 가능 위치 : ", (santa_next_x, santa_next_y))
    ...
    next_x, next_y = (
        santas[idx].x + dx[next_dir],
        santas[idx].y + dy[next_dir],
    )
    ...
    if (rudolf.x, rudolf.y) == (santas[idx].x, santas[idx].y):
        ...
        santas[idx].score += d
        conflict(d, idx, santas[idx].x, santas[idx].y, next_dir ^ 2)
